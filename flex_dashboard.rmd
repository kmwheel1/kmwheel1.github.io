...
### C.Top 8 Ordered Fruits

This bar chart shows the counts of the 8 most popular fruits ordered in the Instacart dataset. 
...
fruit_aisle_data = insta_data %>% 
  ...
  summarize(count = n()) %>%
  arrange(desc(count)) %>% 
  ##over 300 items in the fresh fruit aisle, want to look at top 10 of them (10 was too many, so changing to 8)
  slice(1:8)

fruit_aisle_data %>% 
   plot_ly(x = ~product_name, y = ~count, color = ~product_name, type = "bar") %>%
  layout(title = "Top 8 Ordered Fruits")
